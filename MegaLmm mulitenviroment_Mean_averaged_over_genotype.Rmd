---
title: "The use of MegaLMM in multiple trait analysis - the mean data averaged over genotype"
...
```{r cars}
# Merge datas from different trials 
...
d1 = dcast(data = data_comb, formula = intercept + geno ~ studyName,fun.aggregate = mean, value.var = "Test_weight" ) 
dim(d1)
head(d1)

trtloc_name  = paste(colnames(d1)[3:12],"Test_weight",sep = "_")
colnames(d1)[3:12] = trtloc_name
head(d1)
d2 = dcast(data = data_comb, formula = geno ~ studyName,fun.aggregate = mean, value.var = "Plant_height" )
dim(d2)
head(d2)
trtloc_name  = paste(colnames(d2)[2:11],"Plant_height",sep = "_")
colnames(d2)[2:11] = trtloc_name
head(d2)

d3 = dcast(data = data_comb, formula = geno ~ studyName,fun.aggregate = mean, value.var = "Grain_Yield" )
dim(d3)

trtloc_name  = paste(colnames(d3)[2:11],"Grain_Yield",sep = "_")
colnames(d3)[2:11] = trtloc_name
head(d3)


dat_final = cbind(d1,d2[,-c(1)],d3[,-c(1)])
dim(dat_final)
head(dat_final)
...
#####################################################
head(dat_final)
design_dat = dat_final[, c("intercept","geno")] # subseting the desing matrix 
head(design_dat)

dim(dat_final)
head(dat_final)
Y = dat_final[,3:32] # add the traits in the setup file


summary(Y)
dim(Y)
all(rownames(Kmatrix) %in% design_dat$geno)

# Subset the genotypes in Kmatrix found in phenotye data
idg = which(design_dat$geno %in% rownames(Kmatrix))
design_dat = design_dat[idg,] # The design matrix 
Y = Y[idg,]

idmark = which(rownames(Kmatrix) %in% design_dat$geno)
Kmatrix = Kmatrix[idmark,idmark]
dim(Kmatrix)
all(rownames(Kmatrix) %in% design_dat$geno)
```
...
design_dat$geno = as.factor(design_dat$geno) # change to factor variable 
head(design_dat)
# setup the MegaLMM model 
MegaLMM_state = setup_model_MegaLMM(Y,            # n x p data matrix
                              ~ (1|geno),  # RHS of base model for factors and residuals. Fixed effects defined here only apply to the factor residuals.
